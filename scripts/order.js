// Fortezza Coffee — order page (checkout shell)
// Single location, Fortezza hours, FZ- order IDs.
(() => {
  if (!window.FortezzaCart) return;
  const { read, total, remove, clear, itemTotal } = window.FortezzaCart;
  ...
  // Fortezza Coffee hours per weekday (0=Sun, 6=Sat).
  const HOURS = {
  ...
  form?.addEventListener('submit', (e) => {
    ...
    const orderId = 'FZ-' + Date.now().toString(36).toUpperCase().slice(-6);
